...
image<short> read_frame(const capture_source &source) {
	...
	//draw((short *)source.buffers[buf.index].start, buf.bytesused);
	...
	image<short> img((short *)source.buffers[buf.index].start, 640, 480);
	return img;
}
...
capture_source open_device(const device &d) {
	...
	if (valid_path(dev_name)) {
		...
		capture_source source{fd, buffers};
